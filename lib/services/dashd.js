...
var Proposal = npccoincore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to npccoind in Node.js. Manages starting and
 * stopping npccoind as a child process for application support, as well as connecting
 * to multiple npccoind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function NPCcoin(options) {
  if (!(this instanceof NPCcoin)) {
    return new NPCcoin(options);
  }
...
util.inherits(NPCcoin, Service);

NPCcoin.dependencies = [];

NPCcoin.DEFAULT_MAX_TXIDS = 1000;
NPCcoin.DEFAULT_MAX_HISTORY = 50;
NPCcoin.DEFAULT_MAX_UTXO = 1000;
NPCcoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
NPCcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
NPCcoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
NPCcoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
NPCcoin.DEFAULT_SPAWN_STOP_TIME = 10000;
NPCcoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
NPCcoin.DEFAULT_REINDEX_INTERVAL = 10000;
NPCcoin.DEFAULT_START_RETRY_INTERVAL = 5000;
NPCcoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
NPCcoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
NPCcoin.DEFAULT_INSTANTSEND_FEE = 10000;
NPCcoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  uacomment: 'npccoincore'
};

NPCcoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || NPCcoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || NPCcoin.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || NPCcoin.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || NPCcoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || NPCcoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || NPCcoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || NPCcoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || NPCcoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || NPCcoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || NPCcoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || NPCcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

NPCcoin.prototype._initCaches = function() {
...
NPCcoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
NPCcoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
NPCcoin.prototype.getPublishEvents = function() {
  return [
    {
...
NPCcoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'npccoind/' + name, 'total:', this.subscriptions[name].length);
};

NPCcoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'npccoind/' + name, 'total:', this.subscriptions[name].length);
};

NPCcoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (npccoincore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'npccoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

NPCcoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'npccoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
NPCcoin.prototype.unsubscribeAddressAll = function(emitter) {
...
NPCcoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = NPCcoin.DEFAULT_CONFIG_SETTINGS;
...
NPCcoin.prototype._parseNPCcoinConf = function(configPath) {
  var options = {};
...
NPCcoin.prototype._expandRelativeDatadir = function() {
...
NPCcoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in npccoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in npccoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in npccoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './npccoin.conf');

  log.info('Using NPCcoin config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseNPCcoinConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseNPCcoinConf(networkConfigPath));
  }
...
NPCcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of npccoincore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of npccoincore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of npccoincore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in npccoin.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that npccoind is undergoing a reindex. ' +
             ...
             'of npccoincore-node services will start.');
    node._reindex = true;
  }
};

NPCcoin.prototype._resetCaches = function() {
 ...
 * @private
 */
NPCcoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
NPCcoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

NPCcoin.prototype._initChain = function(callback) {
  var self = this;
...
NPCcoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 7167
  };
  if (this.node.network === npccoincore.Networks.testnet) {
    networkOptions.rpcport = 17167;
  }
  return networkOptions;
};

NPCcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === npccoincore.Networks.testnet) {
    networkPath = 'testnet3/npccoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/npccoin.conf';
    }
  }
  return networkPath;
};

NPCcoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === npccoincore.Networks.testnet) {
...
NPCcoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('npccoind/hashblock', message.toString('hex'));
    }
  }

};

NPCcoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
NPCcoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('NPCcoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
NPCcoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
NPCcoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('npccoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
NPCcoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = npccoincore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('npccoind/rawtransaction', message.toString('hex'));
    }

    var tx = npccoincore.Transaction();
    tx.fromString(message);
    var txid = npccoincore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

NPCcoin.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = npccoincore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('npccoind/transactionlock', message.toString('hex'));
    }

  }
};

NPCcoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || NPCcoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

NPCcoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
NPCcoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
NPCcoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('NPCcoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || NPCcoin.DEFAULT_REINDEX_INTERVAL);

};

NPCcoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
NPCcoin.prototype._stopSpawnedNPCcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/npccoind.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned npccoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean npccoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
NPCcoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedNPCcoin(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting npccoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('NPCcoin process unexpectedly exited with code:', code);
        log.warn('Restarting npccoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('NPCcoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn npccoind.'));
      }
...
NPCcoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to npccoind.'));
    }
 ...
 */
NPCcoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('NPCcoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
NPCcoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, npccoincore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
NPCcoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = npccoincore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
NPCcoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
NPCcoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
NPCcoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in npccoincore is most recent to less recent
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getAddressTxids = function(addressArg, options, callback) {
...
NPCcoin.prototype._getConfirmationsDetail = function(transaction) {
...
NPCcoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
NPCcoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
NPCcoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A npccoin transaction id
 * @param {Function} callback
 */
NPCcoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
NPCcoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof npccoincore.Address) {
...
NPCcoin.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
NPCcoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a NPCcoincore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
NPCcoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = npccoincore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
NPCcoin.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
NPCcoin.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a NPCcoincore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
NPCcoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
NPCcoin.prototype.govObjectList = function(options, callback) {
  var self = this;
...
NPCcoin.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
NPCcoin.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
NPCcoin.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
NPCcoin.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
NPCcoin.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
NPCcoin.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
NPCcoin.prototype.govObjectInfo = function (callback) {
  var self = this;
...
NPCcoin.prototype.govCount = function (callback) {
  var self = this;
...
NPCcoin.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
NPCcoin.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
NPCcoin.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
NPCcoin.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.getInfo = function(callback) {
  var self = this;
...
NPCcoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
NPCcoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('npccoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('npccoind process did not exit'));
      }
