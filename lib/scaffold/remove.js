...
/**
 * Will remove a service from npccoincore-node.json
...
/**
 * Will remove the Node.js service and from the npccoincore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The npccoincore-node configuration directory
...
function remove(options, done) {
  ...
  var npccoincoreConfigPath = path.resolve(configPath, 'npccoincore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(npccoincoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a npccoincore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to npccoincore-node.json
        removeConfig(npccoincoreConfigPath, service, next);
      });
    }, done
  );
}
