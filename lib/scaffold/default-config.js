...
/**
 * Will return the path and default npccoincore-node configuration. It will search for the
 * configuration file in the "~/.npccoincore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.npccoincore');
  var defaultConfigFile = path.resolve(defaultPath, './npccoincore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['npccoind', 'web'];
