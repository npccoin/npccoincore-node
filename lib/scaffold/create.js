...
var BASE_PACKAGE = {
  description: 'A full NPCcoin node build with NPCcoincore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    '@npccoin/npccoincore-lib': '^' + npccoincore.version,
    '@npccoin/npccoincore-node': version
  }
};

/**
 * Will create a directory and npccoin.conf file for NPCcoin.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createNPCcoinDirectory(datadir, done) {
...
/**
 * Will create a base NPCcoincore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The npccoin database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/npccoincore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a NPCcoincore Node directory, configuration file,
 * npccoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the npccoincore node configuration directory
 * @param {String} options.datadir - The path to the npccoin datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the npccoincore-node directory and configuration
    ...
    function(next) {
      // Setup the NPCcoin directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createNPCcoinDirectory(absDataDir, next);
      } else {
        next();
      }
    },
