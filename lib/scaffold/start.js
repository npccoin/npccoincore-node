...
var NPCcoincoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for npccoind is necessary for this upgrade with the "reindex=1" npccoin.conf option. \n' +
                 'There are changes necessary in both npccoin.conf and npccoincore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/npccoin/npccoincore-node/blob/npccoind/docs/upgrade.md \n');
        ...
        servicesConfig: {
          npccoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/npccoind')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed npccoincore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './npccoincore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in npccoincore-node services directory
    ...
    if (servicePackage.npccoincoreNode) {
      serviceModule = service.name + '/' + servicePackage.npccoincoreNode;
    }
 ...
 * @param {Node} node - The NPCcoincore Node instance
 */
