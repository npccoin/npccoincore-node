...
console.log('NPCcoin Service native interface vs. NPCcoin JSON RPC interface');
console.log('----------------------------------------------------------------------');

// To run the benchmarks a fully synced NPCcoin Core directory is needed. The RPC comands
// can be modified to match the settings in npccoin.conf.
...
var npccoind = require('../').services.NPCcoin({
  node: {
    datadir: process.env.HOME + '/.npccoin',
    network: {
      name: 'testnet'
    }
  }
...
npccoind.start(function(err) {
  ...
  console.log('NPCcoin Core started');
});

npccoind.on('ready', function() {

  console.log('NPCcoin Core ready');

  var client = new npccoindRPC({
    host: 'localhost',
    port: 18332,
    user: 'npccoin',
    pass: 'local321'
  ...
  async.series([
    function(next) {
      ...
      function npccoindGetBlockNative(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
      ...
      function npccoindGetBlockJsonRpc(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
        var hash = fixtureData.blockHashes[c];
      ...
      function npccoinGetTransactionNative(deffered) {
        if (c >= txLength) {
          c = 0;
        }
        var hash = fixtureData.txHashes[c];
        npccoind.getTransaction(hash, true, function(err, tx) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      function npccoinGetTransactionJsonRpc(deffered) {
        if (c >= txLength) {
          c = 0;
        }
        var hash = fixtureData.txHashes[c];
      ...
      suite.add('npccoind getblock (native)', npccoindGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('npccoind getblock (json rpc)', npccoindGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('npccoind gettransaction (native)', npccoinGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('npccoind gettransaction (json rpc)', npccoinGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
