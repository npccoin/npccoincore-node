...
To develop bitcore-node-npccoin:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-npccoin.git
...
We now will setup symlinks in `bitcore-node-npccoin` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
...
cd devnode
mkdir node_modules
touch bitcore-node-npccoin.json
touch package.json
```

Edit `bitcore-node-npccoin.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-npccoin/bin/bitcore-node-npccoin start
```