...
describe('NPCcoin Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, process.env.HOME, './.npccoincore/data/npccoind')
  var nodesConf = [
    {
      datadir: path.resolve(__dirname, './data/node1'),
      conf: path.resolve(__dirname, './data/node1/npccoin.conf'),
      rpcuser: 'npccoin',
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node2/npccoin.conf'),
      rpcuser: 'npccoin',
      rpcpassword: 'local321',
      rpcport: 30522,
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node3/npccoin.conf'),
      rpcuser: 'npccoin',
      rpcpassword: 'local321',
      rpcport: 30523,
  ...
  before(function(done) {
    log.info('Starting 3 npccoind daemons');
    this.timeout(200000);
  ...
  it('step 1: will connect to three npccoind daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = npccoincore.Networks.get('regtest');
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.npccoind.once('tip', function(height) {
